...
//! XCM configurations for the Pass3dt runtime.

use super::{
	pass3d_messages::{WithPass3dMessageBridge, DEFAULT_XCM_LANE_TO_PASS3D},
	AccountId, AllPalletsWithSystem, Balances, Call, Event, Origin, Runtime,
	WithPass3dMessagesInstance,XcmPallet,
};
use bp_messages::LaneId;
use bp_pass3dt::WeightToFee;
...
parameter_types! {
	...
	pub const ThisNetwork: NetworkId = CustomNetworkId::Pass3dt.as_network_id();
	/// The Pass3d network ID.
	pub const Pass3dNetwork: NetworkId = CustomNetworkId::Pass3d.as_network_id();
...
/// individual routers.
pub type XcmRouter = (
	// Router to send messages to Pass3d.
	XcmBridgeAdapter<ToPass3dBridge>,
);
...
/// With-Pass3d bridge.
pub struct ToPass3dBridge;

impl XcmBridge for ToPass3dBridge {
	type MessageBridge = WithPass3dMessageBridge;
	type MessageSender = pallet_bridge_messages::Pallet<Runtime, WithPass3dMessagesInstance>;
	...
	fn verify_destination(dest: &MultiLocation) -> bool {
		matches!(*dest, MultiLocation { parents: 1, interior: X1(GlobalConsensus(r)) } if r == Pass3dNetwork::get())
	}

	fn build_destination() -> MultiLocation {
		let dest: InteriorMultiLocation = Pass3dNetwork::get().into();
		let here = UniversalLocation::get();
		dest.relative_to(&here)
	}

	fn xcm_lane() -> LaneId {
		DEFAULT_XCM_LANE_TO_PASS3D
	}
}
...
mod tests {
	...
	fn xcm_messages_are_sent_using_bridge_router() {
		new_test_ext().execute_with(|| {
			...
			// message 1 to Pass3d
			let dest = (Parent, X1(GlobalConsensus(Pass3dNetwork::get())));
			let send_result = send_xcm::<XcmRouter>(dest.into(), xcm.clone());
			assert_eq!(send_result, Ok((expected_hash, expected_fee.clone())));

			// message 2 to Pass3dParachain (expected hash is the same, since other lane is used)
			let dest =
				(Parent, X2(GlobalConsensus(Pass3dNetwork::get()), Parachain(PASS3D_PARACHAIN_ID)));
	...
	#[test]
	fn xcm_messages_from_pass3d_are_dispatched() {
		type XcmExecutor = xcm_executor::XcmExecutor<XcmConfig>;
		...
		new_test_ext().execute_with(|| {
			let location: MultiLocation =
				(Parent, X1(GlobalConsensus(Pass3dNetwork::get()))).into();
